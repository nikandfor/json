...
func (v *Iter) Unmarshal(r interface{}) error {
	rv := reflect.ValueOf(r)
	v.TypeNext()
	return v.unmarshal(rv)
}

func (v *Iter) unmarshal(rv reflect.Value) error {
	//	log.Printf("unmarshal : %v %v %v  '%s'", v.i, v.lasttp, v.err, v.b[v.i:])
...
func (v *Iter) unmarshalStruct(rv reflect.Value) error {
	//	log.Printf("unmarStruc: %v %v %v  '%s'", v.i, v.lasttp, v.err, v.b[v.i:])
	...
	tp := v.Type()
	if tp != ObjStart {
		v.err = ErrExpectedValue
		return v.Err()
	}

	for tp = v.TypeNext(); tp == ObjKey; tp = v.TypeNext() {
		k := v.NextString()
		tp = v.TypeNext()

		//	log.Printf("obj: '%s'  tp %v", k, tp)
...
func (v *Iter) unmarshalArray(rv reflect.Value) error {
	//	log.Printf("unmarshArr: %v %v %v '%s'", v.i, v.lasttp, v.err, v.b[v.i:])
	...
	j := 0
	for tp := v.TypeNext(); tp != ']'; tp = v.TypeNext() {
		//	log.Printf("j: %v  tp: %v  i %d  type %v", j, tp, v.i, elkind)
		var err error
		if j == rv.Cap() {
